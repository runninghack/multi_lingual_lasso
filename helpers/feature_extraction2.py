...
def feature_extraction(language):
    folder_twitter = "C:/data/enriched_data/2014_clean_split/" + language + "/"
    ...
    for d in all_dates:
        print d
        f_path = folder_twitter + datetime.strftime(d, "%Y-%m-%d")
        ...
        if os.path.isfile(f_path):
            ...
            for city in cities:
                ...
                string_line = ", ".join([str(a[1]) for a in sorted(keywords_dict.iteritems())]) + \
                              ", " + datetime.strftime(d, "%Y-%m-%d") + "," + city + "\n"
                fout.write(string_line.encode('utf-8'))
        else:
            for city in cities:
                string_line = "".join(["0, "] * len(keywords_dict)) + datetime.strftime(d, "%Y-%m-%d") + "," + city + "\n"
                fout.write(string_line.encode('utf-8'))

if __name__ == "__main__":
    languages = ['English', 'Portuguese', 'Spanish']
    for l in languages:
        feature_extraction(l)
